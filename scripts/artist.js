...
document.addEventListener('DOMContentLoaded', function(){
  ...
  // ============================================================
  // RANDOM ALBUM SHOWCASE - Display random album under intro video
  // ============================================================
  function initRandomAlbumShowcase() {
    var randomAlbumContainer = document.getElementById('random-album-container');
    var randomAlbumImage = document.getElementById('random-album-image');
    var randomAlbumArtist = document.getElementById('random-album-artist');
    ...
    var randomAlbum = artistAlbums[randomArtistName];
    
    // Update the display
    randomAlbumImage.src = randomAlbum.cover;
    randomAlbumImage.alt = randomArtistName + ' - ' + randomAlbum.albumName;
    if(randomAlbumArtist) randomAlbumArtist.textContent = randomArtistName;
    if(randomAlbumTitle) randomAlbumTitle.textContent = randomAlbum.albumName;
    
    // Add hover effect
    if(randomAlbumCover) {
      randomAlbumCover.addEventListener('mouseenter', function() {
        this.style.transform = 'scale(1.03)';
      });
      randomAlbumCover.addEventListener('mouseleave', function() {
        this.style.transform = 'scale(1)';
      });
      
      // Click to open player
      randomAlbumCover.addEventListener('click', function() {
        if(window.RokkoPlayer && randomAlbum) {
          // Stop all other audio
          document.querySelectorAll('audio').forEach(function(audio) {
            if(!audio.paused) audio.pause();
          });
          
          var playlist = randomAlbum.tracks.map(function(track) {
            return {
              title: track.title,
              artist: randomArtistName,
              audioSrc: track.src,
              coverSrc: randomAlbum.cover
            };
          });
  ...
  // Initialize random album showcase
  initRandomAlbumShowcase();
  ...
  // Audio Player Modal Functions - NOW USES RokkoPlayer
